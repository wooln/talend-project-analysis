...
package export_talend7_context.export_context_0_1;
 ...
 * @status
 */
public class export_context implements TalendJob {
	...
	private final String jobName = "export_context";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					export_context.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(export_context.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class pStruct implements routines.system.IPersistableRow<pStruct> {
		final static byte[] commonByteArrayLock_EXPORT_TALEND7_CONTEXT_export_context = new byte[0];
		static byte[] commonByteArray_EXPORT_TALEND7_CONTEXT_export_context = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_EXPORT_TALEND7_CONTEXT_export_context.length) {
					if (length < 1024 && commonByteArray_EXPORT_TALEND7_CONTEXT_export_context.length == 0) {
						commonByteArray_EXPORT_TALEND7_CONTEXT_export_context = new byte[1024];
					} else {
						commonByteArray_EXPORT_TALEND7_CONTEXT_export_context = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context, 0, length);
				strReturn = new String(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context, 0, length);
				strReturn = new String(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_EXPORT_TALEND7_CONTEXT_export_context) {
	...
	public void tFileList_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String directory_tFileList_1 = context.contextDir;
				...
				for (int i_tFileList_1 = 0; i_tFileList_1 < list_tFileList_1.size(); i_tFileList_1++) {
					...
					if (execStat) {
						runStat.updateStatOnConnection("iterate3", 1, "exec" + NB_ITERATE_tFileInputXML_1);
						// Thread.sleep(1000);
					}
					...
					String fileName_tFileOutputExcel_1 = context.contextDir + "/context_list.xls";
...
// ###############################
					...
					if (isValidFile_tFileInputXML_1) {
						...
						org.dom4j.XPath x_tFileInputXML_1 = doc_tFileInputXML_1.createXPath(nsTool_tFileInputXML_1
								.addDefaultNSPrefix("/xmi:XMI/talendfile:ContextType/contextParameter"));
							...
							public void initXPaths_0(java.util.Map<Integer, org.dom4j.XPath> xpaths,
								...
								org.dom4j.XPath xpath_0 = org.dom4j.DocumentHelper
										.createXPath(nsTool.addDefaultNSPrefix("../@name",
												"/xmi:XMI/talendfile:ContextType/contextParameter"));
								...
								org.dom4j.XPath xpath_4 = org.dom4j.DocumentHelper
										.createXPath(nsTool.addDefaultNSPrefix("@xmi:id",
												"/xmi:XMI/talendfile:ContextType/contextParameter"));
						...
						for (org.dom4j.Node temp_tFileInputXML_1 : nodeList_tFileInputXML_1) {
							...
							try {
								...
								if (xml_api_tFileInputXML_1.isDefNull(node_tFileInputXML_1)) {
									...
									row1.name1 = str_tFileInputXML_1;
								}
...
// ###############################      
					...
					if (execStat) {
						runStat.updateStatOnConnection("iterate3", 2, "exec" + NB_ITERATE_tFileInputXML_1);
					}
	...
	public static void main(String[] args) {
		final export_context export_contextClass = new export_context();

		int exitCode = export_contextClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = export_context.class.getClassLoader().getResourceAsStream(
					"export_talend7_context/export_context_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = export_context.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : export_context");
		}
