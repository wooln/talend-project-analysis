...
package export_talend7_context.export_context_file_id_0_1;
...
/**
 * Job: export_context_file_id Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class export_context_file_id implements TalendJob {
	...
	private final String jobName = "export_context_file_id";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					export_context_file_id.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(export_context_file_id.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class pStruct implements routines.system.IPersistableRow<pStruct> {
		final static byte[] commonByteArrayLock_EXPORT_TALEND7_CONTEXT_export_context_file_id = new byte[0];
		static byte[] commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id.length) {
					if (length < 1024 && commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id.length == 0) {
						commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id = new byte[1024];
					} else {
						commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id, 0, length);
				strReturn = new String(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id, 0, length);
				strReturn = new String(commonByteArray_EXPORT_TALEND7_CONTEXT_export_context_file_id, 0, length,
						utf8Charset);
			}
			return strReturn;
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_EXPORT_TALEND7_CONTEXT_export_context_file_id) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				for (int i_tFileList_1 = 0; i_tFileList_1 < list_tFileList_1.size(); i_tFileList_1++) {
					...
					String fileName_tFileOutputExcel_1 = context.contextDir + "/context_id.xls";
					...
					if (xlsxTool_tFileOutputExcel_1.getStartRow() == 0) {
						...
						xlsxTool_tFileOutputExcel_1.addCellValue("displayName");
...
//init all mapping xpaths
						...
						class XPathUtil_tFileInputXML_1 {
							...
							public void initXPaths_0(java.util.Map<Integer, org.dom4j.XPath> xpaths,
								...
								org.dom4j.XPath xpath_2 = org.dom4j.DocumentHelper.createXPath(nsTool
										.addDefaultNSPrefix("@displayName", "/xmi:XMI/TalendProperties:Property"));
						...
						for (org.dom4j.Node temp_tFileInputXML_1 : nodeList_tFileInputXML_1) {
							...
							try {
								...
								if (xml_api_tFileInputXML_1.isDefNull(node_tFileInputXML_1)) {
									...
									row1.displayName = str_tFileInputXML_1;
								}
...
// # Output table : 'p'
									...
									p_tmp.displayName = row1.displayName;
...
// Start of branch "p"
								if (p != null) {
									...
									if (p.displayName != null) {
										...
										xlsxTool_tFileOutputExcel_1.addCellValue(String.valueOf(p.displayName));
									} else {
										xlsxTool_tFileOutputExcel_1.addCellNullValue();
									}
...
// ###############################      
	...
	public static void main(String[] args) {
		final export_context_file_id export_context_file_idClass = new export_context_file_id();

		int exitCode = export_context_file_idClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = export_context_file_id.class.getClassLoader().getResourceAsStream(
					"export_talend7_context/export_context_file_id_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = export_context_file_id.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : export_context_file_id");
		}
...
/************************************************************************************************
 * 63004 characters generated by Talend Open Studio for Data Integration on the
 * 2023年12月1日 CST 16:06:17
 ************************************************************************************************/